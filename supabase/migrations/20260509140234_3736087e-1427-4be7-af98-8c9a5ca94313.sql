-- Add fingerprint column (one fingerprint per physical device/phone)
ALTER TABLE public.license_keys
  ADD COLUMN IF NOT EXISTS bound_fingerprints text[] NOT NULL DEFAULT '{}'::text[];

-- Reset all bindings so new rule applies cleanly
UPDATE public.license_keys
SET bound_devices = '{}', bound_ips = '{}', bound_fingerprints = '{}';

-- Updated validator: requires fingerprint, enforces 1 physical device per key
CREATE OR REPLACE FUNCTION public.validate_license_key(_key text, _device_id text, _fingerprint text DEFAULT NULL)
...
DECLARE
  ...
  max_fingerprints constant int := 1;
BEGIN
  ...
  -- New fingerprint: only allow if no device bound yet
  ...
  RETURN QUERY SELECT false, 'Key sudah dipakai di perangkat lain. Hubungi admin untuk reset.', current_key.expiry_date;
END;
$function$;

-- Update reset function to also clear fingerprints
